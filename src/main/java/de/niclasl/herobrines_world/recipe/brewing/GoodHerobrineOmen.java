...
@EventBusSubscriber
public class GoodHerobrineOmen implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new GoodHerobrineOmen());
	}
	...
	public boolean isIngredient(@NotNull ItemStack ingredient) {
		return Ingredient.of(Items.RED_DYE).test(ingredient);
	}

	@Override
	public @NotNull ItemStack getOutput(@NotNull ItemStack input, @NotNull ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), ModPotions.GOOD_HEROBRINE_OMEN);
		}
