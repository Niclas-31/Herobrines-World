...
@EventBusSubscriber
public class BadHerobrineOmen implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new BadHerobrineOmen());
	}
	...
	public boolean isIngredient(@NotNull ItemStack ingredient) {
		return Ingredient.of(Items.BLACK_DYE).test(ingredient);
	}

	@Override
	public @NotNull ItemStack getOutput(@NotNull ItemStack input, @NotNull ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), ModPotions.BAD_HEROBRINE_OMEN);
		}
