...
import de.niclasl.herobrines_world.registries.block.custom.HerobrinesRealmPortalBlock;
...
public class HerobrinesRealmPortalShape {
    ...
    public static Optional<HerobrinesRealmPortalShape> findEmptyPortalShape(LevelAccessor level, BlockPos bottomLeft, Direction.Axis axis) {
        return findPortalShape(level, bottomLeft, shape -> shape.isValid() && shape.numPortalBlocks == 0, axis);
    }

    public static Optional<HerobrinesRealmPortalShape> findPortalShape(LevelAccessor level, BlockPos bottomLeft, Predicate<HerobrinesRealmPortalShape> predicate, Direction.Axis axis) {
        Optional<HerobrinesRealmPortalShape> optional = Optional.of(findAnyShape(level, bottomLeft, axis)).filter(predicate);
    ...
    public static HerobrinesRealmPortalShape findAnyShape(BlockGetter level, BlockPos bottomLeft, Direction.Axis axis) {
        ...
        if (blockpos == null) {
            return new HerobrinesRealmPortalShape(axis, 0, direction, bottomLeft, 0, 0);
        } else {
            ...
            if (i == 0) {
                ...
                return new HerobrinesRealmPortalShape(axis, mutableint.intValue(), direction, blockpos, i, j);
            }
        }
    }
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(ModBlocks.HEROBRINES_REALM_PORTAL.get());
    }
    ...
    public void createPortalBlocks(LevelAccessor level) {
        BlockState blockstate = ModBlocks.HEROBRINES_REALM_PORTAL.get().defaultBlockState().setValue(HerobrinesRealmPortalBlock.AXIS, this.axis);
