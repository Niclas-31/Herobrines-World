...
import de.niclasl.herobrines_world.client.renderer.entity.RedCrystalRenderState;
...
public class RedCrystalModel extends EntityModel<RedCrystalRenderState> {
    ...
    public RedCrystalModel(ModelPart root) {
        super(root);
    ...
    @Override
    public void setupAnim(@NonNull RedCrystalRenderState state) {
        super.setupAnim(state);
        this.base.visible = state.showsBottom;
        float animationSpeed = state.ageInTicks * 3.0F;
        float crystalY = RedCrystalRenderer.getY(state.ageInTicks) * 16.0F;
