...
public class ModVillagerProfessions {
	...
	public static final DeferredRegister<VillagerProfession> PROFESSIONS = DeferredRegister.create(Registries.VILLAGER_PROFESSION, HerobrinesWorld.MODID);

	public static final DeferredHolder<VillagerProfession, VillagerProfession> LUMBERJACK = registerProfession("lumberjack", ModBlocks.LUMBERJACK_TABLE,
	...
	private static DeferredHolder<VillagerProfession, VillagerProfession> registerProfession(String name, Supplier<Block> block, Supplier<SoundEvent> soundEvent) {
		...
		return PROFESSIONS.register(name, () -> {
			...
			return new VillagerProfession(Component.translatable("entity.villager." + HerobrinesWorld.MODID + "." + name), poiPredicate, poiPredicate, ImmutableSet.of(), ImmutableSet.of(), soundEvent.get());
		});
	}
		...
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			for (Map.Entry<String, ProfessionPoiType> entry : POI_TYPES.entrySet()) {
				...
				if (existingCheck.isPresent()) {
                    HerobrinesWorld.LOGGER.error("Skipping villager profession {} that uses POI block {} that is already in use by {}", name, block, existingCheck);
					continue;
				}
				PoiType poiType = new PoiType(ImmutableSet.copyOf(block.getStateDefinition().getPossibleStates()), 1, 1);
				registerHelper.register(ResourceLocation.fromNamespaceAndPath("herobrines_world", name), poiType);
